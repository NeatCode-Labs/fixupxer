...
/**
 * Helper that shows a single-choice dialog letting the user pick which TikTok
 * embed proxy the app should use — the TikTok twin of [InstagramProxyDialogHelper].
 * The roster is built dynamically: fixed proxies from [Constants] (Primary/Backup
 * badges) + user-defined custom proxies (Custom badge, deletable) + a trailing
 * "Add custom proxy…" row.
 *
 * Reuses the generic picker layouts/strings shared with the Instagram picker
 * (item_instagram_proxy_option, dialog_add_custom_proxy, badge labels,
 * validation error strings) — only the title/info texts are TikTok-specific.
 *
 ...
 */
object TikTokProxyDialogHelper {
    ...
    fun show(
        ...
        customTitle.findViewById<ImageView>(R.id.tiktokProxyInfoIcon)
        ...
        adapter = ProxyListAdapter(
            ...
            onDeleted = { deletedDomain ->
                val selectedBefore = prefs.getTikTokProxy()
                prefs.removeCustomTikTokProxy(deletedDomain)
                adapter.reload()
                // Deleting the selected proxy silently falls back to the default —
                // let the caller refresh its "Active: <proxy>." label.
                val selectedAfter = prefs.getTikTokProxy()
        ...
        dialog.listView.setOnItemClickListener { _, _, position, _ ->
            when (val row = adapter.getItem(position)) {
                is Row.Proxy -> {
                    prefs.setTikTokProxy(row.domain)
    ...
    private class ProxyListAdapter(
        ...
        private fun buildRows(): List<Row> {
            val fixed = Constants.TIKTOK_PROXY_DOMAINS.map { domain ->
                val badge = if (domain in Constants.TIKTOK_PRIMARY_PROXIES) {
            ...
            val custom = prefs.getCustomTikTokProxies().map { domain ->
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
            ...
            when (val row = rows[position]) {
                is Row.Proxy -> {
                    ...
                    radio.isChecked = row.domain == prefs.getTikTokProxy()
    ...
    private fun showAddCustomProxyDialog(
        ...
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
            val domain = TikTokProxyStore.normalizeCustomProxyInput(input.text?.toString() ?: "")
            val errorRes = when {
                !TikTokProxyStore.isValidProxyDomainFormat(domain) ->
                    R.string.instagram_proxy_error_invalid_domain
                TikTokProxyStore.isReservedDomain(domain) ->
                    R.string.instagram_proxy_error_reserved_domain
                TikTokProxyStore.isDuplicate(domain) ->
            ...
            if (errorRes != null) {
                ...
                prefs.addCustomTikTokProxy(domain)
                onAdded()
                dialog.dismiss()
            }
        }
    ...
    private fun buildTitleView(context: Context): View {
        ...
        val title = TextView(context).apply {
            text = context.getString(R.string.tiktok_proxy_dialog_title)
        ...
        val info = ImageView(context).apply {
            id = R.id.tiktokProxyInfoIcon
            setImageResource(R.drawable.ic_info_outline)
            contentDescription = context.getString(R.string.tiktok_proxy_info_content_desc)
            isClickable = true
            isFocusable = true
