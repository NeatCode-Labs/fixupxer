    ...
    private suspend fun hasMultipleUrls(input: String): Boolean {
        return try {
            withTimeout(50) { // 50ms timeout for URL detection
                ...
                val domains = domainPattern.findAll(input).toList()
                
                // Count dots only in the main domain part (before query parameters)
                val urlParts = input.split("?", "#")
                val mainUrl = urlParts[0]
                ...
                Timber.d("InputValidator: domains.size=${domains.size}, domainDots=$domainDots")
                ...
                val result = protocolCount > 1 || wwwCount > 1 || domains.size > 1 || domainDots > 2 || hasGluedUrls
