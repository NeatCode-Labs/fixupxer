...
- **Settings recreation:** restoring a backup theme no longer reapplies stale theme, handedness or custom-rule controls. Browser settings recreation preserves newer mode and action choices instead of writing old checked states back to preferences.
...
- **Regression coverage:** exact reported share URL, duplicate identifiers, host/key/fragment boundaries, idempotence across MAIN/SHARE/BROWSER, and ShareActivity's cleaned URL and status.
...
### Build Artifacts (v2.6.5, previous release):
