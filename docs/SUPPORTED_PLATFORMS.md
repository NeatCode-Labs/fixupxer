...
- **YouTube** - Removes known YouTube tracking keys, including `si`/`is` share identifiers on YouTube Music. Keeps song IDs, playlist/radio context, timestamps and unknown parameters. Standard YouTube links can convert to experimental Invidious readers: `inv.nadeko.net` (default), `invidious.nerdvpn.de`; Music links keep their Music domain.
